...
def upgrade():
    ...
    op.create_table("traffic_flows", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("capture_time", sa.DateTime(timezone=True)), sa.Column("source_ip", sa.String(45)), sa.Column("source_port", sa.Integer()), sa.Column("destination_ip", sa.String(45)), sa.Column("destination_port", sa.Integer()), sa.Column("protocol", sa.String(30)), sa.Column("raw_features", portable_json, nullable=False), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False))
    ...
    op.create_table("predictions", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("traffic_flow_id", sa.Integer(), sa.ForeignKey("traffic_flows.id", ondelete="CASCADE"), nullable=False, unique=True), sa.Column("model_id", sa.Integer(), sa.ForeignKey("models.id", ondelete="RESTRICT"), nullable=False), sa.Column("predicted_label", sa.String(50), nullable=False), sa.Column("confidence_score", sa.Float(), nullable=False), sa.Column("class_probabilities", portable_json, nullable=False), sa.Column("prediction_time", sa.DateTime(timezone=True), nullable=False), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False), sa.CheckConstraint("predicted_label IN ('Normal', 'DDoS', 'PortScan')", name="ck_predictions_predicted_label"))
    ...
    op.create_table("alerts", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("prediction_id", sa.Integer(), sa.ForeignKey("predictions.id", ondelete="CASCADE"), nullable=False, unique=True), sa.Column("severity", sa.String(20), nullable=False), sa.Column("title", sa.String(200), nullable=False), sa.Column("description", sa.Text(), nullable=False), sa.Column("status", sa.String(30), nullable=False), sa.Column("acknowledged_at", sa.DateTime(timezone=True)), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False), sa.CheckConstraint("severity IN ('HIGH', 'MEDIUM')", name="ck_alerts_severity"), sa.CheckConstraint("status IN ('ACTIVE', 'ACKNOWLEDGED')", name="ck_alerts_status"))
