...
DDOS_INPUT = ROOT / "data/lab/flows/cicflowmeter-v3/ddos-test.pcap_ISCX.csv"
REPORT = ROOT / "reports/metrics/experiment_c_v3_final.json"
NORMAL_TABLE = ROOT / "reports/tables/experiment_c_v3_normal_predictions_final.csv"
PORTSCAN_TABLE = ROOT / "reports/tables/experiment_c_v3_portscan_predictions_final.csv"
DDOS_TABLE = ROOT / "reports/tables/experiment_c_v3_ddos_predictions.csv"
...
def main() -> None:
    outputs = (REPORT, NORMAL_TABLE, PORTSCAN_TABLE, DDOS_TABLE)
    existing_outputs = [path for path in outputs if path.exists()]
    if existing_outputs:
        relative_paths = ", ".join(str(path.relative_to(ROOT)) for path in existing_outputs)
        raise FileExistsError(f"Refusing to overwrite existing final output(s): {relative_paths}")
    ...
    ddos_table, ddos = _evaluate(DDOS_INPUT, "DDoS", adapter, engine)
    ddos["ddos_to_normal"] = ddos["predictions"]["Normal"]
    ddos["ddos_to_ddos"] = ddos["predictions"]["DDoS"]
    ddos["ddos_to_portscan"] = ddos["predictions"]["PortScan"]
    ddos["ddos_recall"] = ddos["predictions"]["DDoS"] / ddos["total"]

    confusion = [
        [normal["predictions"][label] for label in CLASSES],
        [ddos["predictions"][label] for label in CLASSES],
    ...
    correct = (
        ...
        + ddos["predictions"]["DDoS"]
        + portscan["predictions"]["PortScan"]
    )
    total = normal["total"] + ddos["total"] + portscan["total"]
    ...
    top20_ddos = (
        ddos_table.nlargest(20, "P_DDoS")[top20_columns]
        .rename(columns={"predicted_label": "predicted_class"})
        .to_dict(orient="records")
    )
    ...
    report = {
        "status": "completed",
        "experiment_status": "COMPLETED — NORMAL, DDOS, AND PORTSCAN TESTED",
        "inference_path": {
        ...
        "portscan": portscan,
        "confusion_matrix": {
            "label": "Experiment C confusion matrix",
            "actual_rows": list(CLASSES),
            "predicted_columns": list(CLASSES),
            "values": confusion,
        },
        "probability_summary": {
            "Normal": normal["probability_summary"],
            "DDoS": ddos["probability_summary"],
            "PortScan": portscan["probability_summary"],
        },
        "highest_ddos_probability_flows": top20_ddos,
        "highest_portscan_probability_flows": top20_portscan,
        "final_experiment_c_metrics": {
            "normal_recall": normal_recall,
            "ddos_recall": ddos["ddos_recall"],
            ...
            "overall_flow_level_accuracy": overall_accuracy,
            "total_evaluated_flows": total,
            "total_correctly_classified_flows": correct,
        ...
        "inference_failures": [],
        "limitations": [
            "This is a controlled laboratory flow-level evaluation.",
    ...
    ddos_table.to_csv(DDOS_TABLE, index=False)
    ...
    print("\nDDoS results\n------------")
    print("Ground truth: DDoS")
    print(f"Flows: {ddos['total']}")
    print(f"Predicted Normal: {ddos['predictions']['Normal']}")
    print(f"Predicted DDoS: {ddos['predictions']['DDoS']}")
    print(f"Predicted PortScan: {ddos['predictions']['PortScan']}")
    print(f"DDoS recall: {ddos['ddos_recall']:.6f}")
    print("\nPortScan results\n----------------")
    ...
    print(f"DDoS              {confusion[1][0]:6d}  {confusion[1][1]:4d}  {confusion[1][2]:8d}")
    print(f"PortScan          {confusion[2][0]:6d}  {confusion[2][1]:4d}  {confusion[2][2]:8d}")
    print("\nFinal metrics\n-------------")
    print(f"Normal recall: {normal_recall:.6f}")
    print(f"DDoS recall: {ddos['ddos_recall']:.6f}")
    print(f"PortScan recall: {portscan['portscan_recall']:.6f}")
    print(f"Controlled Normal FPR: {normal['controlled_normal_flow_level_false_positive_rate']:.6f}")
    print(f"Overall flow-level accuracy: {overall_accuracy:.6f}")
    print(f"Total evaluated flows: {total}")
    print(f"Correctly classified flows: {correct}")
    print("Inference failures: 0")
    print("\nOld hieulw PortScan\n-------------------")
    print("Normal: 1000")
    print("PortScan: 0")
    ...
    print("\nExperiment C status:\nCOMPLETED — NORMAL, DDOS, AND PORTSCAN TESTED")
