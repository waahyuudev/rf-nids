...
require_session "$SESSION_ID" "$NORMAL_SESSION_PATTERN"
...
set +e
timeout --signal=TERM 120s bash -c '
  for request_number in $(seq 1 30); do
    started=$(date -u +%Y-%m-%dT%H:%M:%SZ)
    curl --http1.1 --max-time 10 --request GET --user-agent "RF-NIDS-Experiment-F-Normal-Validation/1.0" --silent --show-error --output /dev/null --write-out "%{http_code},%{time_total}" "$TARGET_URL" > "$SESSION_DIR/request_${request_number}.result" 2> "$SESSION_DIR/request_${request_number}.stderr"
    status=$?
    printf "%s,%s,%s\n" "$request_number" "$started" "$status" >> "$SESSION_DIR/requests.csv"
    [[ $status -eq 0 ]] || exit $status
    [[ $request_number -eq 30 ]] || sleep 1
  done
'
readonly STATUS=$?
set -e
