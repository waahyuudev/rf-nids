...
def predictions_table(rows: list[dict]) -> None:
    if not rows:
        ...
        return
    fields = ["prediction_time", "source_ip", "source_port", "destination_ip", "destination_port", "protocol", "predicted_label", "confidence_score"]
    frame = pd.DataFrame(rows).reindex(columns=fields).rename(columns={"prediction_time": "Time", "source_ip": "Source IP", "source_port": "Src Port", "destination_ip": "Destination IP", "destination_port": "Dst Port", "protocol": "Protocol", "predicted_label": "Class", "confidence_score": "Confidence"})
